...
c_filter = input("Geef rgb waarde om weg te filterem [r,g,b]: ")
c_filter = c_filter.split(",")
r_filter = int(c_filter[0])
g_filter = int(c_filter[1])
b_filter = int(c_filter[2])

for i in range(r_in.shape[0]): 
    for j in range(r_in.shape[1]): 
        if r_in[i][j] == r_filter and g_in[i][j] == g_filter and b_in[i][j] == b_filter:
            r_out[i][j] = 0 
            g_out[i][j] = 0 
            b_out[i][j] = 0 
        else:
